...
from app.api_utils import (
    ...
    check_user_channel_limit, get_channel_last_message_seq, get_channel_last_interaction_seq, ephemeral_ttls
)
from app.config import generate
from app.routers.stream import channel_added, channel_edited, channel_deleted, member_join, member_leave, emit, dm_unhide
from app.config import config
from app.db import SQLite

channels_bp=Router("channels")
...
def channels(db:SQLite, id):
    user_channels=db.execute_raw_sql("""
        SELECT c.id, c.type, c.default_ttl,
               ...
               CASE WHEN c.type=1 THEN other_u.is_bot ELSE 0 END as is_bot,
...
def channel_creation(db:SQLite, id):
    ...
    if channel_type==1:
        ...
        target_user=db.select_data("users", ["id", "is_bot", "owner_id", "bot_private"], {"username": request.form["target_user"]})
        ...
        if target_user[0]["is_bot"] and target_user[0]["bot_private"] and target_user[0]["owner_id"]!=id: return make_json_error(403, "You can only DM private bots you own")
        ...
        try:
            ...
            with db:
                ...
                db.insert_data("members", {"user_id": id, "channel_id": channel_id, "joined_at": timestamp(), "message_seq": message_seq, "interaction_seq": interaction_seq})
                db.insert_data("members", {"user_id": target_id, "channel_id": channel_id, "joined_at": timestamp(), "message_seq": message_seq, "interaction_seq": interaction_seq})
...
def channels_management(db:SQLite, id, channel_id):
    if request.method=="PATCH":
        ...
        if "ttl" in request.form:
            ...
            if ttl=="off": update_data["default_ttl"]=None
            elif ttl in ephemeral_ttls: update_data["default_ttl"]=ephemeral_ttls[ttl]
            else: errors.append("Invalid ttl parameter")
        ...
        updated_channel=db.execute_raw_sql("SELECT id, name, pfp, type, permissions, default_ttl FROM channels WHERE id=?", (channel_id,))[0]
...
def join_invite(db:SQLite, id, invite_code):
    if db.exists("users", {"id": id, "is_bot": 1}): return make_json_error(403, "Bots cannot join channels themselves; a member with manage-members permission must add them")
    ...
    db.insert_data("members", {"user_id": id, "channel_id": channel_id, "joined_at": timestamp(), "message_seq": 0 if channel_type==3 else get_channel_last_message_seq(db, channel_id), "interaction_seq": 0 if channel_type==3 else get_channel_last_interaction_seq(db, channel_id)})
...
@channels_bp.route("/channel/<string:channel_id>/bot", methods=["POST"])
@logged_in()
@sliding_window_rate_limiter(limit=20, window=60, user_limit=10)
@validate_request_data({"bot": {"minlen": 1, "maxlen": 30}})
def add_bot(db:SQLite, id, channel_id):
    # Discord-style: a bot never joins on its own; a member with manage-members permission adds a bot they own.
    member_data=db.execute_raw_sql("""
        SELECT m.permissions, c.type, c.permissions as channel_permissions
    ...
    data=member_data[0]
    if data["type"]==1: return make_json_error(400, "Cannot add bots to DM channels")
    if not has_permission(data["permissions"], perm.manage_members, data["channel_permissions"]): return make_json_error(403, "Manage members permission required")
    bot_ref=request.form["bot"].lstrip("@")
    bot=db.select_data("users", ["id", "username", "display_name", "pfp", "owner_id", "is_bot", "bot_private"], {"id": bot_ref})
    if not bot: bot=db.select_data("users", ["id", "username", "display_name", "pfp", "owner_id", "is_bot", "bot_private"], {"username": bot_ref})
    if not bot or not bot[0]["is_bot"]: return make_json_error(404, "Bot not found")
    bot=bot[0]
    if bot["bot_private"] and bot["owner_id"]!=id: return make_json_error(403, "You can only add bots you own")
    if db.exists("members", {"user_id": bot["id"], "channel_id": channel_id}): return make_json_error(400, "Bot is already in this channel")
    if db.exists("bans", {"user_id": bot["id"], "channel_id": channel_id}): return make_json_error(403, "Bot is banned from this channel")
    # Bots are exempt from the channel member cap: they neither count toward it nor get blocked by it
    requested_perms=0
    if request.form.get("permissions", "").strip():
        try: requested_perms=int(request.form["permissions"])
        except ValueError: requested_perms=0
    granted_perms=requested_perms&perm.mask&~perm.owner&~perm.admin
    granted_perms=sum(bit for bit in (perm.send_messages, perm.manage_messages, perm.manage_members, perm.manage_channel, perm.manage_permissions) if granted_perms&bit and has_permission(data["permissions"], bit, data["channel_permissions"]))
    if not granted_perms and has_permission(data["permissions"], perm.send_messages, data["channel_permissions"]): granted_perms=perm.send_messages
    db.insert_data("members", {"user_id": bot["id"], "channel_id": channel_id, "joined_at": timestamp(), "permissions": granted_perms, "message_seq": 0 if data["type"]==3 else get_channel_last_message_seq(db, channel_id), "interaction_seq": 0 if data["type"]==3 else get_channel_last_interaction_seq(db, channel_id)})
    user_data={"id": bot["id"], "username": bot["username"], "display_name": bot["display_name"], "pfp": bot["pfp"], "is_bot": 1}
    member_join(channel_id, user_data, db)
    return jsonify({"success": True})
