...
from app.responses import jsonify, make_response
...
ephemeral_ttls={"10s": 10000, "1m": 60000, "1h": 3600000, "24h": 86400000}
...
def get_channel_last_interaction_seq(db: SQLite, channel_id: str) -> int:
    result=db.execute_raw_sql("SELECT MAX(seq) as last_seq FROM interaction_history WHERE channel_id=?", (channel_id,))
    return result[0]["last_seq"] if result and result[0]["last_seq"] is not None else 0
    ...
    def decorator(f):
        ...
        def wrapper(db, *args, **kwargs):
            ...
            if get:
                ...
                if not data:
                    if len(token)!=30: return make_json_error(401, "Unauthorized")
                    bot_data=db.select_data("bot_tokens", ["id", "bot_id"], {"token_hash": hash_token(token)})
                    if not bot_data: return make_json_error(401, "Unauthorized")
                    data={"user": bot_data[0]["bot_id"], "id": f"bot:{bot_data[0]['id']}"}
                else: data=data[0]
...
device_links={}
device_links_lock=Lock()

oauth_codes={}
oauth_codes_lock=Lock()
...
def cleaner():
    ...
    while not stopping.is_set():
        ...
        with device_links_lock:
            for code in list(device_links):
                if device_links[code]["expire"]<now: del device_links[code]
        with oauth_codes_lock:
            for code in list(oauth_codes):
                if oauth_codes[code]["expire"]<now: del oauth_codes[code]
        ...
        def wrapper(*args, **kwargs):
            with lock:
                ...
                if "Authorization" in request.headers:
                    ...
                    if len(auth_header_split)>=2 and auth_header_split[0]=="Bearer" and len(auth_header_split[1]) in (20, 30):
                        db=SQLite()
                        if len(auth_header_split[1])==30:
                            bot_data=db.select_data("bot_tokens", ["bot_id"], {"token_hash": hash_token(auth_header_split[1])})
                            if bot_data: user_id=bot_data[0]["bot_id"]
                        else:
